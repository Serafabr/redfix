...
create table supplies (
  ...
  bdi numeric not null default 1,
  final_price numeric
);

create table task_supplies (
  -- binding WHAT is to be used...
  task_id integer not null references tasks (task_id),
  spec_id integer not null references supplies (supply_id) on delete cascade,
  -- specifies WHICH BOX is to take the supply from...
  box_id integer references boxes (box_id),
  -- supply_id (for convenience) -- will be populated automatically when user defines which box
  supply_id integer references supplies (supply_id),
  invoice_id integer,-- references invoies (invoice_id),
  qty_proposed numeric,
  qty_approved numeric,
  qty_consumed numeric,
  -- no more primary key in this table (same task is allowed to have same supply more than once)
);
...
alter table task_supplies add foreign key (invoice_id) references invoices (invoice_id) on delete set null;

...
--   name text not null,
--   description text,
--   category_id integer not null references assets (asset_id),
...
create table depots (
  ...
  url text,
  sigad text,
  box_id integer-- references boxes (box_id) SEE ALTER TABLE COMMAND BELOW
);
...
alter table depots add foreign key (box_id) references boxes (box_id);
...
create table specs (
  ...
  ext_ref text, -- improve (new table is necessary?)
...
create table spec_prices (
  spec_id integer primary key references specs (spec_id),
  created_at timestamptz not null default now(),
